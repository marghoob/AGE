...
void AGEaligner::traceBackMaxima(int &lg1,int &lg2,int &rg1,int &rg2)
{
  ...
  while (tr != 0) {
    if (tr == FM_VERTICAL) {
      ...
      pos--;
      lg1--;
    } else cerr<<"Internal error (1) in tracking."<<endl;
  ...
  while (tr != 0) {
    if (tr == RM_VERTICAL) {
      ...
      pos++;
      rg1++;
    } else cerr<<"Internal error (2) in tracking."<<endl;
    tr = trace[pos] & RM_MASK;
  }
}
...
int AGEaligner::addBpoints(int lg1,int lg2,int rg1,int rg2)
{
  ...
  //cerr<<"Buffer for breakpoints exceeded. Skipping ..."<<endl;
  return -1;
}
...
Block *AGEaligner::findLeftBlocks(int left1,int left2)
{
  ...
  while (tr != 0) {
    if (tr == FS_DIAGONAL) {
      ...
      left2--;
      pos -= score_n1;
    } else cerr<<"Internal error (3) in tracking."<<endl;
...
Block *AGEaligner::findRightBlocks(int right1,int right2)
{
  ...
  while (tr != 0) {
    ...
    } else cerr<<"Internal error (4) in tracking."<<endl;
